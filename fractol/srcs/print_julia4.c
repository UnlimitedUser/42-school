...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/01/18 22:56:37 by dsclearo          #+#    #+#             */
/*   Updated: 2017/01/20 13:13:43 by dsclearo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/fractol.h"

void	julia4_itter(t_event *event, t_point *p, t_complex complex,
		int *iteration)
{
	t_point		a;
	double		x_new;
...
void	print_julia4(t_event *event)
{
	...
	while (++p.y < WIN_HEIGHT)
	{
		p.x = -1;
		while (++p.x < WIN_WIDTH)
			julia4_itter(event, &p, complex, &iteration);
	}
}
