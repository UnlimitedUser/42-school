...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/01/18 13:07:10 by dsclearo          #+#    #+#             */
/*   Updated: 2017/01/18 13:07:11 by dsclearo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/fractol.h"

void	imaginary_itter(t_event *event, t_point *p, t_complex complex,
						int *iteration)
{
	t_point		a;
	double		x_new;
	...
	*iteration = 0;
	while (a.x * a.x + a.y * a.y <= 4 && *iteration < MAXIMUM)
	{
		...
		a.x = 2 * fabs(a.y) * fabs(a.x) + complex.imag;
		a.y = x_new;
		(*iteration)++;
	}
...
void	print_imaginary(t_event *event)
{
	...
	while (++p.y < WIN_HEIGHT)
	{
		p.x = -1;
		while (++p.x < WIN_WIDTH)
			imaginary_itter(event, &p, complex, &iteration);
	}
}
